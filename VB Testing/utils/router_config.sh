...
configure_tcp_cc_ecn() {
    ...
    if [ "$ecn_status" == "ecn" ]; then
        ...
        ssh -p "$dsthostport" -i "$sshkeypath" root@"$vmhostaddr" "sysctl net.inet.tcp.ecn.enable=$tcp_ecn_enable"
        ...
        ssh -p "$dsthostport" -i "$sshkeypath" root@"$vmhostaddr" "sysctl net.inet.tcp.ecn.enable=0"
    fi

    # Set DCTCP ECT1
    if [ "$tcp1" == "dctcp" ]; then
        if [ "$ecn_status" == "ecn" ]; then
            ssh -p "$router1port" -i "$sshkeypath" root@"$vmhostaddr" "sysctl net.inet.tcp.ecn.enable=3"
            ssh -p "$src1port" -i "$sshkeypath" root@"$vmhostaddr" "sysctl net.inet.tcp.ecn.enable=3"
            ssh -p "$src2port" -i "$sshkeypath" root@"$vmhostaddr" "sysctl net.inet.tcp.ecn.enable=3"
            ssh -p "$dsthostport" -i "$sshkeypath" root@"$vmhostaddr" "sysctl net.inet.tcp.ecn.enable=3"
        fi
