...
#     ssh -p "$src2port" -i "$sshkeypath" root@"$vmhostaddr" "iperf3 -c 172.16.1.2 -t $duration -p 5101 -J -C cubic > iperf3_client_cubic_${testname}.json" &
...
udp_client_iperf3_script() {
    ...
    ssh -p "$dsthostport" -i "$sshkeypath" root@"$vmhostaddr" "stdbuf -oL ./udp_prague/udp_prague_receiver -p 5101 > udp_prague_receiver_${testname}.txt" &
    
    sleep 2
    ssh -p "$src1port" -i "$sshkeypath" root@"$vmhostaddr" "stdbuf -oL ./udp_prague/udp_prague_sender -a 172.16.1.2 -p 5101 -c > udp_prague_sender_${testname}.txt" &
    ssh -p "$src2port" -i "$sshkeypath" root@"$vmhostaddr" "iperf3 -c 172.16.1.2 -t $duration -p 5103 -J -C cubic > iperf3_client_cubic_${testname}.json" &
    ssh -p "$src2port" -i "$sshkeypath" root@"$vmhostaddr" "iperf3 -c 172.16.1.2 -t $duration -p 5102 -J > iperf3_client_${tcp2}_${testname}.json" 
    

    ssh -p "$dsthostport" -i "$sshkeypath" root@"$vmhostaddr" "cd udp_prague; pkill -f udp_prague_receiver"
