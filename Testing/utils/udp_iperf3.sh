...
udp_client_iperf3_script() {
    ...
    ssh -i ~/.ssh/mptcprootkey root@$server_ipaddr "./udp_prague/udp_prague_receiver -p 5101 > udp_prague_receiver_${testname}.txt" &
    
    sleep 2
    ssh -i ~/.ssh/mptcprootkey root@$client1_ipaddr "./udp_prague/udp_prague_sender -a 192.168.3.2 -p 5101 -c > udp_prague_sender_${testname}.txt" &
